...
def render_page(context: RagPageContext) -> None:
    ...
    with st.chat_message("user"):
        st.write(question)
    with st.chat_message("assistant"), st.spinner(strings["RAG_GENERATING"]):
        answer = conversational_answer(
            index.run_dir, question, state, config, history=history, cached_chunks=cached
        )
    ...
    _append_history(context.session_root(), index, config, question, answer)
    st.rerun()
...
def _append_history(
    session_root, index, config, question: str, answer: ConversationalAnswer
) -> None:
